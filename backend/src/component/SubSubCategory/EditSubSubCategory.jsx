...
export default function EditSubSubCategory(){
    ...
    useEffect(() => {
        axios.get('/api/sub-sub-categories/get-category-list').then(response => {
            if(response.data.status === 200){
                setParentCategories(response.data.subsubcategories)
                setLoading(false)
            }
    ...
    useEffect(() => {
        axios.get(`/api/sub-sub-categories/edit/${id}`).then(response => {
            if(response.data.status == 200){
                setCategoryInfo(response.data.category)
                setLoading(false)
            }else if(response.data.status == 404){
                Swal.fire('404',response.data.message,'error')
                navigate('/admin/sub-sub-categories',{
                    replace: true
                })
            }
        }).catch(error => {
    ...
    const formSubmit = (e) => {
        ...
        axios.post('/api/sub-sub-categories/update',categoryInfo).then(response => {
