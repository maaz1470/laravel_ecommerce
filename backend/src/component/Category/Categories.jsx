...
export default function Categories(){
  ...
  const deleteCategory = (e,id) => {
    e.preventDefault();
    axios.get(`/api/categories/delete/${id}`).then(response => {
  ...
  const statusChange = (e,status) => {
    e.preventDefault()
    axios.get(`/api/categories/change-status/${id}`)
  }
    ...
    return (
        <>
            <div className="flex justify-between items-center">
                ...
                <Link to="/admin/add-category" className="bg-green-400 text-white px-3 py-2 h-max rounded text-md">Add Category</Link>
            </div>
            ...
            <div className="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
              <div className="w-full overflow-x-auto">
                <table className="w-full whitespace-no-wrap">
                  ...
                  >
                    {
                      categoriesInfo.toReversed().map(el => (
                        <tr className="text-gray-700 dark:text-gray-400" key={Math.random()}>
                            ...
                            <div className="flex justify-between w-1/3">
                              {
                                ...
                                <Link to={`/admin/category/edit/${el.id}`} className="bg-blue-600 px-3 py-2 rounded text-white"><i className="fa fa-pencil" aria-hidden="true"></i></Link>
                                <a onClick={(e) => deleteCategory(e,el.id)} href="" className="bg-red-500 px-3 py-2 rounded text-white"><i className="fa fa-trash" aria-hidden="true"></i></a>
                            </div>
                          </td>
                        </tr>
                      ))
